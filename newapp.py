...
def test_corpusEmbedding():
    ...
    t = stoplist
    vectorSize = [25, 50, 100, 200, 300]
    windowSize = [2, 3, 5]
    for vec in vectorSize:
        for win in windowSize:
            for columnN in [LISTOFPROPERTIES, "description"]:
                trainingModel(t, dataSetFile,
                              columnN, 1, vec, win, 1, 3, None, "row", "Texts")
...
def test_createFrequnecyModel():
    ...
    for attrib in listOfAttributs:
        completeSimilarityOfDatasets("myModel.bin", "tfidf", listOfKBCSVFile[0], listOfWordsImportance[0], listOfKBCSVFile[1],
